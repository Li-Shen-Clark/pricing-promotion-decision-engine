...
\begin{itemize}
  ...
  \item Streamlit app，包括 Overview、Model Evidence、What-If Simulator、Candidate Finder、Test Planner、Trust \& Boundaries 和 Upload \& Score 七个页面（v0.2 重命名后）；
...
页面 & 用户操作 & 输出 \\
\midrule
Model Evidence (\texttt{/Evidence}) & 查看模型结果 & 自身弹性、交叉弹性、sale-code log-point effect、holdout error、识别 caveat \\
What-If Simulator (\texttt{/Simulate}) & 调整 effective price、promo、competitor price、cost、inventory & capped sold units、收入、利润、变化率、风险提示 \\
Candidate Finder (\texttt{/Optimize}) & 点击 rerun optimizer 或调整 scenario & $p^*$、$m^*$、$\Delta Revenue$、$\Delta Profit$、upper-guardrail flag \\
Test Planner (\texttt{/Validate}) & 查看候选实验或输入 MDE / power & 样本量、实验周期、guardrail metrics、underpowered flag \\
Trust \& Boundaries (\texttt{/Boundaries}) & 阅读模型限制 & 价格/促销内生性、AAC cost proxy、库存场景化、raise-and-test framing \\
...
Trust \& Boundaries / model card 页面 & 数据限制、识别限制、适用范围、guardrail binding 风险 & 建议 \\
